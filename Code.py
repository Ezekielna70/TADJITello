...
center_margin   = 20       # px toleransi horizontal
vertical_margin = 20       # px toleransi vertical
...
def queue_cmd(cmd, arg, label):
    """Flush queue lalu push cmd terbaru; update teks."""
...
def yolo_thread(model, drone):
    ...
    global prev_move, running

    while running:
        try: frame = frame_queue.get(timeout=0.2)
        except queue.Empty: time.sleep(0.01); continue
        if frame is None: time.sleep(0.01); continue

        res = model.predict(frame, conf=0.3)
        names = res[0].names if len(res)>0 else []
        latest_boxes = [(b,float(b.conf[0]),names[int(b.cls[0])])
                        for b in res[0].boxes] if names else []

        move_needed = "diam"                              # default
        ...
        if move_needed != prev_move:
            ...
            if   move_needed=="kiri":   queue_cmd("move_left" ,20,"kiri")
                ...
                set_last("diam")                 # hanya update overlay, tak kirim cmd
                while not command_queue.empty(): # pastikan queue kosong
...
def camera_thread(drone):
    ...
    while running:
        ...
        if camera_ready_count>=CAMERA_READY_THRESHOLD and not takeoff_done:
            queue_cmd("takeoff",None,"Takeoff"); time.sleep(3); takeoff_done=True

        frames+=1; fps=frames/max(1e-6,time.time()-fps_start)
        small=cv2.resize(frame,(FRAME_W,FRAME_H))
        if not frame_queue.full(): frame_queue.put(cv2.cvtColor(small,cv2.COLOR_BGR2RGB))

        disp=small.copy()
        for b,conf,c in latest_boxes:
            x1,y1,x2,y2=map(int,b.xyxy[0])
            cv2.rectangle(disp,(x1,y1),(x2,y2),(0,255,0),2)
            cv2.putText(disp,f"{c} {conf:.2f}",(x1,y1-10),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,255,0),2)
        ...
        status="(Manual Mode)" if manual_override else "(Belum Takeoff)" if not takeoff_done else last_action
        cv2.putText(disp,f"FPS:{fps:.1f}",(10,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
        cv2.putText(disp,status,(10,70),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)

        cv2.imshow("TelloCam",disp)
        if cv2.waitKey(1)&0xFF==ord('q'):
            running=False; queue_cmd("land",None,"Landing"); break
    cv2.destroyAllWindows()

# =========== MANUAL CONTROL THREAD ============
def manual_control():
    global running, manual_override
    key_map={'w':("move_forward",30,"maju"), 's':("move_back",30,"mundur"),
             'a':("move_left",30,"kiri"), 'd':("move_right",30,"kanan"),
             'i':("move_up",30,"naik"), 'k':("move_down",30,"turun"),
             'j':("rotate_ccw",30,"rot kiri"), 'l':("rotate_cw",30,"rot kanan"),
             '9':("flip_left",None,"flip kiri"), '0':("flip_right",None,"flip kanan"),
             't':("takeoff",None,"Takeoff"), 'q':("land",None,"Landing")}
    while running:
        if keyboard.is_pressed('o'):
            manual_override=not manual_override
            set_last("(Manual Mode)" if manual_override else "(Auto Track)"); time.sleep(0.4)
...
def main():
    global frame_read
    drone=Tello(); drone.connect(); print("[INFO] Batt:",drone.get_battery())
    drone.streamon(); frame_read=drone.get_frame_read()
    model=YOLO(r"D:\Kuliah_ITS\Semester_8\TA Kelar Amin\Code\Git\best.pt")

    th=[ threading.Thread(target=command_thread,args=(drone,)),
    ...
    for t in th: t.start()
    for t in th: t.join()
    drone.streamoff(); drone.end(); print("[INFO] Selesai")
